...
export default async function Home() {
  ...
  return (
    <section className="mx-auto flex max-w-3xl flex-col gap-3 px-6 py-12">
      <h1 className="text-2xl font-semibold text-zinc-950 dark:text-white">
        Welcome back, {session.user.displayName ?? "buyer"}.
      </h1>
      <p className="text-sm text-zinc-600 dark:text-zinc-300">
        Search, garage, and the four-path comparison surfaces are owned by
        Teams 4–8 and will land on this route as they merge. The shell, bell,
        and inbox mount points above are live now.
      </p>
    </section>
  );
}
